...
Questa guida spiega come utilizzare lo script PowerShell `converti_md_pdf_completo.ps1` per convertire file Markdown in PDF professionali, e come personalizzare l'output modificando i file CSS e JSON. La pipeline usa un pre-processore Node.js che converte le formule LaTeX in HTML KaTeX statico (nessun JavaScript viene eseguito durante il rendering del PDF).

---

## Quick Guide (English)

This project provides a small, script-based pipeline to convert Markdown files to PDF on Windows.

- **What it does**
  - Reads all `*.md` files in the `.md/` folder.
  - Pre-renders LaTeX math (`$...$`, `$$...$$`) to **KaTeX HTML** using `convert-md-to-pdf.js` (Node.js).
  - Uses `npx md-to-pdf` (Marked + Puppeteer/Chromium) with `style.css` and `pdf-options.json` to generate PDFs.
  - Moves the resulting PDFs to the `.pdf/` folder and cleans up temporary files.

- **Input / output**
  - Input Markdown: `.md/your-file.md` (never committed to git).
  - Output PDF: `.pdf/your-file.pdf`.

- **Math syntax (what you write in Markdown)**
  - Inline: `$E = mc^2$`
  - Block:
  ...
  - The pre-processor converts these formulas to KaTeX HTML before `md-to-pdf` runs.

- **How to run**
  1. Put your `.md` files into the `.md` folder.
  ...
  3. PDFs will appear in `.pdf/` and the folder will open automatically in Explorer.

- **Customization**
  - Visual style: edit `style.css` (page size, fonts, tables, code, math).
  - PDF layout, margins, header/footer: edit `pdf-options.json` (Puppeteer PDF options).

Your personal Markdown/PDF content stays on your machine and is never committed to the public repository.
...
Lo script `converti_md_pdf_completo.ps1` converte automaticamente tutti i file `.md` presenti nella cartella `.md/` in file PDF, utilizzando:

- **Pre-processore Node.js** (`convert-md-to-pdf.js`): legge il Markdown originale, converte le formule LaTeX (`$...$`, `$$...$$`) in HTML KaTeX statico e scrive il risultato in un file temporaneo (`nomefile.__pdf__.md`).
- **CSS** (`style.css`): per lo styling del contenuto, incluse le formule matematiche (tramite KaTeX).
- **PDF Options** (`pdf-options.json`): per le impostazioni di layout e formattazione del PDF.

### Requisiti

- PowerShell 5.1 o superiore
- Node.js installato (con `npm`)
- Accesso a `npx` per eseguire `md-to-pdf` senza installazione globale
...
2. **Pre-processing formule matematiche**: per ogni `.md` esegue `convert-md-to-pdf.js`, che legge il file sorgente, converte le formule LaTeX (`$...$`, `$$...$$`) in HTML KaTeX statico e scrive il risultato in un file temporaneo `nomefile.__pdf__.md` all'interno della stessa cartella `.md/`.
3. **Processa tutti i file `.md`**: converte ogni file Markdown (escluso `cursor.md`) con `npx md-to-pdf`, usando il file temporaneo come input, insieme a `style.css` e `pdf-options.json`.
4. **Genera PDF**: crea un file PDF temporaneo `nomefile.__pdf__.pdf` accanto al file temporaneo `.md`, quindi lo sposta in `.pdf/nomefile.pdf` e pulisce i file temporanei.
